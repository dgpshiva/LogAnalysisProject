...
def mostPopularArticleAuthors():
    ...
    query = '''
                SELECT authors.name,
                       sum(TitleAndNumberOfViews.NumberOfViews) AS TotalViewPerAuthor
                FROM authors
                INNER JOIN articles
...
def lotOfErrorsDays():
    ...
    query = '''
                SELECT
                    ...
                    (CAST (errorCounts AS FLOAT)/CAST(totalCounts AS FLOAT))
                      * 100 AS errorPercentage
                FROM
                (
                    SELECT time::timestamp::date AS theDate, count(*) AS errorCounts
                    FROM log
                    WHERE status != '200 OK'
                ...
                INNER JOIN
                (
                    SELECT time::timestamp::date AS theDate, count(*) AS totalCounts
                    FROM log
                    GROUP BY time::timestamp::date
                ) AS dateAndTotalCounts
                ON dateAndErrorCounts.theDate = dateAndTotalCounts.theDate
                WHERE ( CAST (errorCounts AS FLOAT) / CAST(totalCounts AS FLOAT) )  * 100 > 1
                ORDER BY ( CAST (errorCounts AS FLOAT) / CAST(totalCounts AS FLOAT) )  * 100 DESC, dateAndTotalCounts.theDate;
            '''
    cursor.execute(query)
    rows = cursor.fetchall()
